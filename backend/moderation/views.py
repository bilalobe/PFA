...
class ModerationViewSet(viewsets.ModelViewSet):
    ...
    ]  # At minimum, authentication is required
    ...
    def get_queryset(self):
        ...
        if self.request.user.user_type == "teacher":
            return Moderation.objects.filter(
                content_object__course__instructor=self.request.user
            )
        else:
...
def moderation_dashboard(request):
    ...
    """
    pending_moderations = Moderation.objects.filter(action_taken="none").order_by(
        "-created_at"
    )
...
def take_action(request, moderation_id):
    ...
    try:
        ...
        return Response(
            {"detail": "Moderation report not found."}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    if serializer.is_valid():
        ...
        return Response(
            {"message": "Moderation action taken."}, status=status.HTTP_200_OK
        )
    else:
...
def report_content(request):
    ...
    if not all([content_type_id, object_id, reason]):
        return Response(
            {"error": "content_type, object_id, and reason are required fields."},
    ...
    if Moderation.objects.filter(
        ...
        return Response(
            {"detail": "You have already reported this content for this reason."},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def notify_moderators(moderation):
    ...
    return Response(
        ModerationSerializer(moderation).data, status=status.HTTP_201_CREATED
    )
