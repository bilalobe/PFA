...
from .tasks import (
    send_enrollment_email_task,
    send_progress_update_email_task,
...
class EnrollmentViewSet(viewsets.ModelViewSet):
    ...
    def calculate_progress(self, enrollment):
        ...
        return (
            (completed_modules + completed_quizzes) / (total_modules + total_quizzes)
        ) * 100
    ...
    def update_progress(self, enrollment):
        try:
            ...
            # Log the error
            logger.error(
                f"Failed to update progress for enrollment {enrollment.id}: {e}"
            )
