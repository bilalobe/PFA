...
class EnrollmentModelTests(TestCase):
    ...
    def test_enrollment_str(self):
        ...
        self.assertEqual(str(enrollment), f"{self.user.username} enrolled in {self.course.title}")
...
class UserViewSetTests(APITestCase):
    def setUp(self):
        ...
        self.user = get_user_model().objects.create_user(
            username='testuser', email='testuser@example.com', password='testpassword', user_type='student'
        )
        self.teacher = get_user_model().objects.create_user(
            username='testteacher', email='testteacher@example.com', password='testpassword', user_type='teacher'
        )
    ...
    def test_update_another_user_profile(self):
        ...
        """
        self.client.force_authenticate(user=self.user)
        url = reverse('user-detail', args=[self.teacher.id])  # Try to update the teacher's profile
        updated_data = {'bio': 'Updated bio'}
        response = self.client.patch(url, updated_data)
