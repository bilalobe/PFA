...
class QuizModelTests(TestCase):
    def setUp(self):
        self.user = get_user_model().objects.create_user(
            username='testuser', email='testuser@example.com', password='testpassword', user_type='teacher'
        )
        self.course = Course.objects.create(title='Test Course', description='Test Description', instructor=self.user)
        self.quiz = Quiz.objects.create(course=self.course, title='Test Quiz', description='Test Quiz Description', created_by=self.user)
