...
const ForumThreadPage = () => {
  ...
  // Function to analyze sentiment when a new post is created
  const analyzePostSentiment = async (postId: string) => {
    try {
      // Call your sentiment analysis Cloud Function
      const response = await aiApi.getSentiment(postId);
      const sentiment = response.sentiment;
      // ... Here you can:
      //       1. Update the post document in Firestore with the sentiment (optional).
      //       2. Take actions based on the sentiment (e.g., if it's very negative, flag it for moderation).
      if (sentiment) {
        await updateDoc(doc(db, `courses/${courseId}/forums/${forumId}/threads/${threadId}/posts/${postId}`), {
          sentiment: sentiment
        });
      }
    } catch (error) {
  ...
  // Call the sentiment analysis function when a new post is added
  useEffect(() => {
    if (posts) {
      posts.forEach(post => {
        if (!post.sentiment) { // Check if sentiment is already analyzed
          analyzePostSentiment(post.id);
        }
      });
    }
  }, [posts]);
