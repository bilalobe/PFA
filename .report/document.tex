...
\begin{itemize}[leftmargin=*,label=\textbullet,font=\color{violet}]
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[node distance=2cm, auto]
            ...
            % Annotations
            \node [above=0.5cm of adapt, align=center] (adaptText) {};
            \node [above=0.5cm of feedback, align=center] (feedbackText) {};
        \end{tikzpicture}
...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}[
		...
		% Updated Milestones for Fictional Scenario
		\node[milestone, below=of planning, yshift=-1.5cm] (idea) {Idéation et Étude de Faisabilité};
		\node[milestone, right=of idea] (funding) {Financement et Partenariats};
		\node[milestone, right=of funding] (prototype) {Création de Prototype et Premiers Tests};
		\node[milestone, below=of prototype, yshift=-1cm] (market) {Introduction sur le Marché et Premières Ventes};
		\node[milestone, left=of market] (expansion) {Expansion et Diversification};
		
		% Arrows - Reflecting the Project Flow for Original Milestones
		...
		% Arrows - Reflecting the Project Flow for Updated Milestones
		\draw[arrow] (idea) -- (funding);
		\draw[arrow] (funding) -- (prototype);
		\draw[arrow] (prototype) -- (market);
		\draw[arrow] (market) -- (expansion);
		\draw[arrow] (idea) edge[bend right] (expansion);
		
		% Linking the two sets of milestones
		\draw[arrow, dashed] (planning) -- (idea);
		\draw[arrow, dashed] (design) -- (funding);
		\draw[arrow, dashed] (development) -- (prototype);
		\draw[arrow, dashed] (testing) -- (market);
		\draw[arrow, dashed] (launch) -- (expansion);
		
	\end{tikzpicture}
	\caption{Charte de PERT pour les étapes du projet}
