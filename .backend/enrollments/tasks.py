...
from backend.enrollments.utils import (
    send_email,
    generate_certificate,
    trigger_follow_up_survey,
)
...
def send_welcome_email_task(self, enrollment_id):
    ...
    try:
        ...
        logger.info(
            f"Welcome email sent to {enrollment.student.email} for enrollment id {enrollment_id}."
        )
...
def send_enrollment_email_task(self, enrollment_id):
    ...
    try:
        ...
        logger.info(
            f"Enrollment confirmation email sent to {
                    enrollment.student.email} for enrollment id {enrollment_id}."
        )
    except Enrollment.DoesNotExist:
        logger.error(f"Enrollment with id {enrollment_id} does not exist.")
    except Exception as exc:
...
def send_progress_update_email_task(enrollment_id):
    ...
    try:
        enrollment = Enrollment.objects.select_related("student").get(id=enrollment_id)
        send_email(
            enrollment.student.email, "Your Course Progress Update", "progress_update"
        )
        logger.info(
            f"Progress update email sent to {
                    enrollment.student.email} for enrollment id {enrollment_id}."
        )
    except Enrollment.DoesNotExist:
        logger.error(f"Enrollment with id {enrollment_id} does not exist.")
...
def send_completion_email_task(enrollment_id):
    ...
    try:
        ...
        if enrollment.completed:
            ...
            logger.info(
                f"Completion email and certificate sent to {
                        enrollment.student.email} for enrollment id {enrollment_id}."
            )
    except Enrollment.DoesNotExist:
        logger.error(f"Enrollment with id {enrollment_id} does not exist.")
